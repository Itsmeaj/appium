...
class AtSpi2Driver extends BaseDriver {
  ...
  async createSession (...args) {
    ...
    try {
      spawnSync('pkill', ['-f', appBaseName], {timeout: 3000});
    } catch { /* ignore */ }
    await wait4sec(0.5);
    ...
    const isAppRunning = () => {
      ...
      try {
        const res = spawnSync('pgrep', ['-f', appBaseName], {encoding: 'utf8', timeout: 3000});
    ...
    // Wayland (especially on RHEL/GNOME) may take longer for windows to register
    // in the AT-SPI tree after launch. Use more retries with longer waits.
  ...
  async deleteSession () {
    if (this.appName && this._backendApis) {
      ...
      try {
        spawnSync('pkill', ['-f', baseName], {timeout: 3000});
      } catch { /* ignore */ }
    }
